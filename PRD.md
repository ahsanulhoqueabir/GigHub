...
8. [Repository & Branch Structure](#8-repository--branch-structure)
9. [External Integration Requirements](#9-external-integration-requirements)
10. [Constraints, Assumptions, and Risks](#10-constraints-assumptions-and-risks)
11. [Feasibility Analysis](#11-feasibility-analysis)
12. [Economic Analysis and Break-Even](#12-economic-analysis-and-break-even)
13. [Agile Scrum Sprint Plan](#13-agile-scrum-sprint-plan)
14. [Testability and Acceptance Strategy](#14-testability-and-acceptance-strategy)
15. [Approval Note](#15-approval-note)
...
### 7.1 Non-Functional Requirements (Simplified)
...
## 8. Repository & Branch Structure

The project follows a **branch-per-platform** strategy to keep codebases isolated:

| Branch         | Contents                                                                                          |
| -------------- | ------------------------------------------------------------------------------------------------- |
| `main`         | Planning & documentation only — `PRD.md`, `project-brief.md`, `readme.md`, optional PDF reports   |
| `web`          | Next.js monorepo (Turborepo) — `apps/web` (frontend) + `apps/api` (API routes) + `@gig-hub/types` |
| `app`          | React Native mobile application                                                                   |
| `shared-types` | Standalone npm package `@gig-hub/types` — Zod validation schemas + TypeScript type definitions    |

### 8.1 Shared Types Package (`@gig-hub/types`)

- All Zod schemas and TypeScript interfaces are defined once in `shared-types`
- Published as `@gig-hub/types` npm package
- Consumed by both `web` (Next.js) and `app` (React Native)
- Ensures form validation, API request/response shapes, and DB row types are consistent everywhere

## 9. External Integration Requirements

### 9.1 Identity & Database Provider (Supabase)

- **Supabase Auth** for email/password and Google OAuth — managed identity provider with built-in session handling
- **Supabase PostgreSQL** as the primary database — tables use descriptive names without prefix
- **Supabase Realtime Broadcast** for real-time chat and live event notifications — no separate WebSocket server required
- Server-side session verification on every protected API call
- Row-Level Security (RLS) policies for data access control

### 9.2 File and Media Storage

- **Cloudflare R2** (S3-compatible object store) for **all** user-uploaded content — documents, assets, delivery files, chat attachments, and original image files; served via Cloudflare's own CDN edge
- **Cloudinary** as image CDN for **image-specific** optimization only — banners, gig images, avatars (resize, crop, format conversion, compression)
- R2 and Cloudinary are independent: R2 handles everything, Cloudinary handles only image optimization
- File size limit: 25 MB per upload
- Signed or access-controlled URLs for sensitive delivery files

### 9.3 Payment Gateways

- **SSLCommerz** as the sole primary payment gateway with secure callback handling
- All payment callbacks must be validated before triggering financial state changes

### 9.4 Real-Time Communication

- Real-time bidirectional messaging via **Supabase Realtime Broadcast** — no Socket.IO server needed
...
| ------------------- | ------------------------------------------------------------------------------------------------------ |
| Backend API         | Next.js API Routes — unified codebase for frontend + backend, reduces context-switching                |
| Authentication      | Supabase Auth — managed identity provider with built-in session handling, reduces custom auth overhead |
| Real-time Chat      | Supabase Realtime Broadcast — no separate WebSocket server to maintain; PostgreSQL-native              |
| Payment Integration | SSLCommerz provides a documented Bangladesh-specific API                                               |
| Mobile              | React Native — TypeScript shared with web, single codebase for Android & iOS                           |
| File Storage        | Cloudflare R2 (all files, served via Cloudflare CDN) + Cloudinary (image-only CDN for optimization)    |
| Shared Types        | `@gig-hub/types` npm package ensures type safety across web & mobile via Zod schemas                   |
| Escrow Logic        | Custom escrow logic is well-understood; primary complexity lies in edge-case dispute handling          |

**Key Technical Risks:**

- Escrow integrity under concurrent payment callbacks requires careful idempotency design
- Supabase Realtime Broadcast connection management at scale (1,000+ concurrent users) needs load-testing validation
- Shared type package versioning across web and mobile branches requires coordinated releases
...
- Admin tooling via Supabase RLS + custom admin panel reduces custom admin development cost
...
_Detailed calculations in Section 12._

### 11.5 Cost-Benefit Analysis (Feasibility View)
...
## 12. Economic Analysis and Break-Even
...
### 12.2 Initial Investment (Year 0 — Pre-Launch)
...
### 12.3 Monthly Operating Cost Model
...
### 12.4 User Adoption Projections
...
### 12.5 Revenue Projections (Paid Transactions Only)
...
### 12.6 Annual Profit / Loss Summary
...
## 13. Agile Scrum Sprint Plan

### 13.1 Scrum Framework Configuration

| Parameter               | Value                                                                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| Sprint duration         | 2 weeks                                                                                                                              |
| Team size               | 5 members (1 Next.js full-stack, 1 React Native, 1 shared-types/pkg lead, 1 QA, 1 design/UX)                                         |
...
| Definition of Done      | Feature code merged to respective branch, Zod schemas in `@gig-hub/types`, unit tested, reviewed, deployed to staging, QA-signed off |
| Backlog tool            | Linear / Jira                                                                                                                        |

### 13.2 Epic and Phase Map

| Phase                       | Sprints | Duration    | Core Epics                                         | FR Coverage      |
| --------------------------- | ------- | ----------- | -------------------------------------------------- | ---------------- |
| Phase 1 — Foundation        | 1–2     | Weeks 1–4   | Auth, Profiles, `@gig-hub/types` scaffolding       | FR-A, FR-B       |
...
| Phase 4 — Communication     | 9–10    | Weeks 17–20 | Real-time Chat (Supabase Broadcast), Notifications | FR-G             |
| Phase 5 — Trust and Quality | 11–13   | Weeks 21–26 | Reviews, Tutoring, Search Polish, Admin, Dashboard | FR-H, FR-I, FR-J |

### 13.3 Phase 1 — Foundation (Sprints 1–2, Weeks 1–4)

- Sprint 1 — Authentication (Supabase Auth setup, `@gig-hub/types` base schemas)
- Sprint 2 — Profiles + shared types refinement

### 13.4 Phase 2 — Marketplace Core (Sprints 3–5, Weeks 5–10)

- Sprint 3 — Gig Creation and Management
- Sprint 4 — Gig Discovery and Search
- Sprint 5 — Job Board and Proposals

### 13.5 Phase 3 — Transactions (Sprints 6–8, Weeks 11–16)

- Sprint 6 — Order Lifecycle
- Sprint 7 — Delivery and Revision Flow
- Sprint 8 — Payments and Escrow (SSLCommerz)

### 13.6 Phase 4 — Communication (Sprints 9–10, Weeks 17–20)

- Sprint 9 — Real-Time Chat (Supabase Realtime Broadcast)
- Sprint 10 — Notifications and Push

### 13.7 Phase 5 — Trust, Quality, and Tutoring (Sprints 11–13, Weeks 21–26)

- Sprint 11 — Reviews and Trust System
- Sprint 12 — Tuition Feature and Search Polish
- Sprint 13 — Admin Tools, Dashboard, and UAT

### 13.8 Sprint Summary Table

| Sprint    | Phase   | Focus                    | Story Points | Duration     |
| --------- | ------- | ------------------------ | ------------ | ------------ |
| Sprint 1  | Phase 1 | Authentication + types   | 38           | Weeks 1–2    |
...
## 14. Testability and Acceptance Strategy
...
This Requirements Analysis Document (v3.0) supersedes RAD v2.0 (April 15, 2026).

Updates in v3.0:

- **Stack migration:** NestJS → Next.js API Routes, Firebase Auth → Supabase Auth, Flutter → React Native, Directus → Supabase PostgreSQL, Socket.IO → Supabase Realtime Broadcast
- **New services:** Cloudinary (image CDN) added alongside Cloudflare R2
- **Shared types package:** `@gig-hub/types` (Zod + TypeScript) added as a cross-platform dependency
- **Repository structure:** Branch-per-platform strategy (`main`, `web`, `app`, `shared-types`) documented
- **Payment simplification:** SSLCommerz as sole payment gateway (bKash/Stripe removed)
- **Section 8** added: Repository & Branch Structure
...
- **Product Owner** — Sections 4, 6, 13
- **Tech Lead** — Sections 7, 8, 9, 10, 13
- **QA Lead** — Sections 6 (acceptance criteria), 14
- **Finance/Admin** — Section 12
