...
function HeroBannerDetailsContent() {
  ...
  const { getItem, deleteItem } = useHeroBannersStore();
  ...
  const handleDelete = useCallback(async () => {
    const confirmed = await confirmDelete(banner?.title || "this banner");
    if (!confirmed) return;
    await deleteItem(id);
      ...
      <div className="flex items-center justify-between space-x-4">
        ...
        {banner && (
          <div className="flex items-center gap-2">
            ...
            <Button asChild>
              <Link
                href={`/admin/hero-banners/${id}/update?returnTo=${encodeURIComponent(returnUrl)}`}
              >
                Edit Banner
              </Link>
            </Button>
...
export default function HeroBannerDetailsPage() {
  return (
    <Suspense fallback={<DetailsSkeleton />}>
      <HeroBannerDetailsContent />
    </Suspense>
  );
}
