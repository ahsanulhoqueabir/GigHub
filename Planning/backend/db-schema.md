...
5. [gh_jobs](#5-gh_jobs)
6. [gh_proposals](#6-gh_proposals)
7. [gh_orders](#7-gh_orders)
8. [gh_order_milestones](#8-gh_order_milestones)
9. [gh_order_deliveries](#9-gh_order_deliveries)
10. [gh_escrow](#10-gh_escrow)
11. [gh_transactions](#11-gh_transactions)
12. [gh_withdrawals](#12-gh_withdrawals)
13. [gh_conversations](#13-gh_conversations)
14. [gh_messages](#14-gh_messages)
15. [gh_reviews](#15-gh_reviews)
16. [gh_notifications](#16-gh_notifications)
17. [gh_bookmarks](#17-gh_bookmarks)
18. [gh_reports](#18-gh_reports)
19. [gh_platform_config](#19-gh_platform_config)
...
- `idx_gigs_seller` on `seller`
- `idx_gigs_category` on `category`
...
| `gig`            | `uuid`          | FK → `gh_gigs.id`, NOT NULL, ON DELETE CASCADE | Parent gig                                      |
...
- UNIQUE(`gig`, `tier`) — one package per tier per gig
...
| `poster`          | `uuid`          | FK → `gh_profiles.id`, NOT NULL   | Creator of the job                                         |
| `category`        | `uuid`          | FK → `gh_categories.id`, NOT NULL | Job category                                               |
...
- UNIQUE(`job`, `applicant`) — one proposal per student per job
- CHECK: `applicant` ≠ job's `poster` (enforced at application level)

**Indexes:**

- `idx_proposals_job` on `job`
- `idx_proposals_applicant` on `applicant`
...
| `buyer`               | `uuid`          | FK → `gh_profiles.id`, NOT NULL | Student who is paying                               |
| `seller`              | `uuid`          | FK → `gh_profiles.id`, NOT NULL | Student who is delivering                           |
| `source_type`         | `varchar(10)`   | NOT NULL                        | Enum: `gig`, `job`                                  |
| `gig`                 | `uuid`          | FK → `gh_gigs.id`, NULL         | Source gig (if source_type = 'gig')                 |
| `gig_package`         | `uuid`          | FK → `gh_gig_packages.id`, NULL | Selected package                                    |
| `job`                 | `uuid`          | FK → `gh_jobs.id`, NULL         | Source job (if source_type = 'job')                 |
| `proposal`            | `uuid`          | FK → `gh_proposals.id`, NULL    | Accepted proposal                                   |
...
- `idx_orders_buyer` on `buyer`
- `idx_orders_seller` on `seller`
- `idx_orders_status` on `status`
- `idx_orders_gig` on `gig`
- `idx_orders_job` on `job`
- `idx_orders_number` on `order_number`

---

## 8. gh_order_milestones
...
| `milestone`     | `uuid`        | FK → `gh_order_milestones.id`, NULL | Related milestone (if applicable)                 |
| `seller`        | `uuid`        | FK → `gh_profiles.id`, NOT NULL     | Delivering student                                |
...
- `idx_transactions_profile` on `profile`
- `idx_transactions_order` on `order`
...
- UNIQUE(`participant_1`, `participant_2`, `order`) — one conversation per pair per order context

**Indexes:**

- `idx_conversations_p1` on `participant_1`
- `idx_conversations_p2` on `participant_2`
- `idx_conversations_order` on `order`
...
- `idx_messages_conversation` on `conversation`
- `idx_messages_sender` on `sender`
- `idx_messages_created` on `created_at`
- `idx_messages_unread` on `conversation, is_read` WHERE `is_read = false`
...
- `idx_notifications_profile` on `profile`
- `idx_notifications_unread` on `profile, is_read` WHERE `is_read = false`
